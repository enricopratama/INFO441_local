...
async function loadUsers(){
    ...
    let usersHTML = usersJson.map(userInfo => {
        const favoriteBands = userInfo.favorite_bands || [];
        return `
        <hr>
        <div>
            <h3>
            ...
            <strong>Favorite Bands:</strong> ${favoriteBands} <br>
...
/**
 * Adding a playlist for a user with a given ID
...
/**
 * Adding a band for a user with a given ID (must include ID to know which user to add the band to)
 * @param {*} id The user ID
 */
async function addBand(id){
    const bandToAdd = document.getElementById("add_band_text_" + id).value; // from userInfo._id (auto-gen from mongodb)
