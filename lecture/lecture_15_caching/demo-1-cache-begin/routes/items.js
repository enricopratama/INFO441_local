...
async function getItemsSlow(req) {
    ...
    let sleepSeconds = 5;
    // Must wait for this to finish first before returningf
...
router.get("/", async (req, res) => {
    ...
    if (allItems) {
        console.log("Cache hit: found items in cache");
    } else {
        // artificially slowed getting items from DB
        console.log("Cache miss: did not find items in cache, doing the slow db lookup now");   
    ...
    // Client-side caching:
    // Stores it into cache but only for 30 seconds max (browser-side caching)
